...
getter는 메소드를 호출하여 값을 구합니다.
  ...
  - getter는 소괄호()를 작성하지 않고 name만 작성합니다.
  - 파라미터를 사용할 수 없습니다.

클래스에 getter 작성 방법 [코드1]
*/
...
setter는 메소드를 호출하여 값을 설정합니다.
  - setter도 getter처럼 소괄호()를 작성하지 않고 이름만 작성합니다.

클래스에 setter 작성 방법 [코드2]
*/
...
static 메소드 작성 방법 [코드3]

static 메소드의 구조적 특징
  - prototype이 아닌 클래스에 연결되며
  - 생성한 인스턴스에 할당되지 않습니다. [코드4]
*/
...
클래스는 호이스팅 hoisting 되지 않습니다.
  - const, let 변수처럼
  - class 키워드가 작성된 위치에서 클래스 이름 선언과 오브젝트 생성을 동시에 하기 때문 [코드5]

const obj = Point;
  - 코드 아래에 Point 클래스가 있지만 Point를 참조하지 못하므로 에러 발생

Point.getPoint();
  - 코드 앞에서 Point 클래스를 오브젝트로 생성하므로 getPoint()를 호출할 수 있습니다.
*/
...
new.target 프로퍼티는
  - 함수 또는 생성자가 new 연산자로 호출된 여부를 반환합니다.

new 연산자로 constructor를 호출하면 new.target은 constructor를 참조 [코드6]

함수로 호출하면 undefined 반환 [코드7]
*/
