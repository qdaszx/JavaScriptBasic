...
[[GetPrototypeOf]]의 트랩입니다.
  - target의 prototype을 반환합니다. [코드1]
  - target이 확장 불가라도 prototype을 반환합니다.
  - String, Number처럼 값을 반환하면 TypeError

트랩 준수 사항
  - target이 확장 불가일 때
  - Object.getPrototypeOf(target)와 같은 값을 반환해야 합니다.
*/
...
Object.setPrototypeOf() 특징
  - target의 __proto__에 두 번째 파라미터를 설정합니다. [코드4]

Object.setPrototypeOf()의 트랩입니다. [코드5]

트랩 준수 사항
  - target이 확장 불가일 때
  - 두 번째 파라미터의 prototype과 Object.getPrototypeOf(target)로 구한 것과 같아야 합니다.
*/
console.log("[코드4] Object.setPrototypeOf()")
class Book4 { setTitle() { return "책" } };
class Point4 { getPoint() { return 100; } };
...
class Book5 { setTitle() { return "책" } };
class Point5 { getPoint() { return 100 } };
const handler5 = {
