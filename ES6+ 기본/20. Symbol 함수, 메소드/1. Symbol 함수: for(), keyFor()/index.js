...
- 글로벌 Symbol 레지스트리(registry)에
  ...
  - 파라미터()의 문자열이 key가 되고 Symbol()로 생성한 값이 value가 됨 [코드1]
  - registry의 사전적 의미: 등록, 기록

- 글로벌 Symbol 레지스트리는 공유 영역
  - 다른 오브젝트에서도 사용 가능
  - 같은 key가 존재하면 등록된 값을 사용 [코드2]
*/
...
형태 ::: Symbol.keyFor()
파라미터 ::: 검색할 Symbol
변환 ::: Symbol key 값, undefined

- 글로벌 Symbol 레지스트리에서 Symbol의 key 값을 구합니다.
  - 파라미터에 Symbol.for()로 등롣한 Symbol 작성 [코드3]

- Symbol key 값이 존재하면
  - key 값을 반환하고 존재하지 않으면 undefined 반환
*/
